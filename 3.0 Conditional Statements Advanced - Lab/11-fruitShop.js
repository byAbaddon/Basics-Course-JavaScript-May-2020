function fruitShop(...arg) {
 [item, day, count] = [arg[0], arg[1],arg[2]]
  ...
  let weekendShop = {banana: 2.70, apple: 1.25, orange: 0.90, grapefruit: 1.60, kiwi: 3.00, pineapple: 5.60, grapes: 4.20 }
   ...
   if (days.indexOf(day) === -1 || shop.hasOwnProperty(item) === false ) { 
      console.log('error');
      return
    }
   console.log(days.indexOf(day) <= 4 ? (shop[item] * count).toFixed(2) : (weekendShop[item] * count).toFixed(2) );
}
